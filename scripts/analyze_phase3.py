...
수행 내용:
  ...
  2. Kruskal-Wallis: manual/random/optuna/autoresearch 4그룹 비교
  3. Mann-Whitney U: Autoresearch vs Optuna 쌍별 비교 (RQ3 핵심 비교)
...
def _run_level_accuracies(results_tsv: Path) -> dict[str, list[float]]:
...
def analyze(results_dir: Path) -> dict:
    ...
    run_level = _run_level_accuracies(results_tsv)
    ...
    if len(present) < 2:
        raise SystemExit(
            ...
            f"현재 데이터 있는 전략: {list(present.keys())} ({results_tsv})"
        )
    ...
    # 3) Mann-Whitney: Autoresearch vs Optuna (RQ3 핵심 쌍별 비교)
    if "autoresearch" in present and "optuna" in present:
        report["mann_whitney_autoresearch_vs_optuna"] = run_mann_whitney(
            present["autoresearch"], present["optuna"]
        )
...
def _render_markdown(report: dict) -> str:
    lines = [
        ...
        "- trial-level 데이터는 탐색 궤적 시각화 전용이며 본 비교 검정에는 사용하지 않음"
        " (설계서 §4.5 v0.5 변경 — sequential optimization의 trial 간 의존성)",
        "",
    ]
    if report["missing_strategies"]:
        lines.append(
            f"> **주의**: 완료 데이터가 없는 전략: {', '.join(report['missing_strategies'])}"
        )
        lines.append("")
    ...
    mw = report["mann_whitney_autoresearch_vs_optuna"]
    lines.append("## Mann-Whitney U — Autoresearch vs Optuna (RQ3 핵심 쌍별 비교)")
    lines.append("")
    if mw:
        sig = "유의함 (p < 0.05)" if mw["significant"] else "유의하지 않음"
        lines.append(
            f"- U = {mw['u_stat']:.4f}, p = {mw['p_value']:.4g}, "
            f"rank-biserial r = {mw['rank_biserial_r']:.4f} -> {sig}"
        )
        lines.append(
            "- r > 0: Autoresearch가 Optuna보다 확률적으로 우세하는 경향 "
            "(부호 규약: `src/evaluate/statistics.py` run_mann_whitney 참조)"
        )
    else:
        lines.append("> Autoresearch 또는 Optuna 완료 데이터 부족으로 생략")
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 3 RQ3 분석 (Kruskal-Wallis + Mann-Whitney + Bootstrap CI, run-level)"
    )
    ...
    md_path = results_dir / "phase3_rq3_analysis.md"
    json_path = results_dir / "phase3_rq3_analysis.json"
