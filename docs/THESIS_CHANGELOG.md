...
## Internal v0.11 — 2026-07-25

**설계서-RUNPOD_GUIDE 정합성 검토** — 설계서와 `RUNPOD_GUIDE.md`를 대조 검토하며 발견한 모순·누락 4건 반영. 코드 변경 없음, 문서 전용 정정.

### 반영 내용

- **§4.5 max_steps 탐색/고정 모순 해소**: 탐색 공간 표는 max_steps를 탐색 가능한 값({100,200,400,800})으로 표기했으나, 고정 조건 문단은 "200 고정"이라고 서술해 서로 모순됐다. 실제 코드(`src/autoresearch/strategies.py`)는 탐색 가능한 버전으로 구현돼 있어 RandomSearch/Optuna가 trial마다 다른 max_steps를 뽑고 있었음을 확인 — 코드를 고정 조건에 맞춰 수정(`PHASE3_FIXED_MAX_STEPS=200`)하고, 탐색 공간 표에서 max_steps 행 제거(9→8개 파라미터).
- **`time_budget_min` 은닉 컨파운드 발견·수정**: 학습 콜백에 걸린 wall-clock 안전장치(기존 15분)가 max_steps보다 먼저 trial을 끊어 사실상 숨은 시간 예산으로 작동하고 있었음 — Phase 2 main 실측 처리량(스텝당 약 13초) 기준 200스텝 완주에는 최소 35-45분이 필요해 기존 15분으로는 거의 모든 trial이 조기 절단됐을 가능성이 높음. 90분으로 상향해 안전장치가 실험 통제 변수를 침범하지 않도록 정정.
- **§5.3 ECE 산출 불가 한계점 신규**: §4.4.5가 ECE를 보조 지표로 제시하나, 평가 파이프라인이 per-sample confidence를 저장하지 않아 항상 "N/A"로 산출됨을 명시.
- **§5.3 Ablation 일반화 서술 정정**: "SLAKE rank=8,16,32 보조 검증으로 일관성만 확인"이라는 문장이 실행 근거(git 이력·결과 파일) 없이 완료된 것처럼 서술돼 있었음을 발견·정정.
- **RUNPOD_GUIDE.md 보강**: 누락돼 있던 cross-dataset CF(Table 4.2b-B) 실행 단계와 Phase 3 RQ3 통계 분석(`analyze_phase3.py`) 사용법 추가.

### 코드 산출물

없음 — 문서 전용 정정(`docs/THESIS_PROPOSAL_FINAL_v0.11.md`, `docs/RUNPOD_GUIDE.md`, `docs/NEXT_SESSION.md`).

### 지도교수 확인 필요

없음 — 설계서와 구현/실행 가이드 간 정합성 정정으로, 실험 방법론(RQ/통계 검정) 자체의 변경은 아님.
