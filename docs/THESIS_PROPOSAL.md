...
| Florence-2-large | 0.77B | DaViT + BERT, 다중 과제 | ~4-6 GB |
...
**실험 조건**:
- 4개 모델 × 3개 데이터셋 = 12개 조건
- 각 조건 3회 반복 (프롬프트 순서 shuffle)
- 동일 프롬프트 템플릿 사용
...
- 훈련 시간 (총 분)
- 훈련 가능 파라미터 비율 (%)
- 범용 성능 감소율 (Catastrophic Forgetting 측정용)
...
- 2.1 Vision-Language Model 개요
  - 2.1.1 멀티모달 학습의 발전
  - 2.1.2 경량 VLM 아키텍처 (Qwen-VL, Florence-2 등)
...
- 3.2 실험 환경 및 도구
  - 3.2.1 하드웨어 사양
  - 3.2.2 소프트웨어 스택 (Unsloth, HuggingFace, Optuna 등)
- 3.3 대상 모델 및 선정 기준
- 3.4 데이터셋 및 전처리
...
| 환경 재현 | requirements.txt / pyproject.toml + CUDA 버전 명시 |
| 실험 추적 | Weights & Biases 또는 results.tsv (autoresearch 스타일) |
...
| 모델 | 조건 | PathVQA Acc. | SLAKE Acc. | VQA-RAD Acc. | 향상율 |
|------|------|:---:|:---:|:---:|:---:|
...
├── src/                        # 소스 코드
│   ├── evaluate/               # 평가 스크립트
│   │   ├── vqa_metrics.py      # VQA accuracy, F1 등
│   │   └── forgetting.py       # Catastrophic forgetting 측정
│   ├── finetune/               # 파인튜닝 스크립트
│   │   ├── train_qlora.py      # QLoRA 학습 메인
│   │   └── prepare_data.py     # 데이터 전처리
│   ├── baseline/               # 베이스라인 평가
│   │   └── zero_shot_eval.py   # 제로샷 평가
