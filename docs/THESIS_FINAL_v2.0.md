...
> 진행 상태: **제1-5장 + 참고문헌 + 부록 A-C 전체 초안 완성.** 3단계 실험(Phase 1 제로샷 12조건 / Phase 2 QLoRA 75조건 / Phase 3 자율 HPO 810 trial) 모두 완료되어 실측값으로 작성됨. 2장 인용은 WebSearch 검증 완료(설계서의 Min-K% 인용 오류 NAACL→ICLR 발견·정정), 참고문헌 23건은 원문 대조 완료.
...
앞서 검토한 세 흐름을 본 연구의 문제의식에 비추어 다시 정리한다.

**의료 특화 VLM 연구(2.3.3)는 대규모 모델과 대규모 도메인 데이터를 전제로 한다.** LLaVA-Med [9], Med-Flamingo [10], CheXagent [11]는 모두 수십억 파라미터 이상의 모델에 대규모 의생명 코퍼스의 계속 사전학습이나 대규모 instruction 데이터 생성을 결합한다. 이 계열은 의료 VQA의 성능 상한을 끌어올렸으나, 소비자급 GPU 한 장으로 재현할 수 있는 접근은 아니다.

**경량 모델을 PEFT로 적응시키려는 연구가 뒤이어 등장했다.** Shourya 등 [18]은 3B급 PaliGemma-mix-448의 시각 타워와 언어 모델 양쪽 어텐션에 LoRA를 적용하고 SLAKE·PMC-VQA·ROCO v2.0·MedPix 2.0을 2단계로 학습시켜, 적절히 큐레이션된 데이터가 있으면 소형 모델도 개방형·폐쇄형 문항 양쪽에서 견고한 성능에 이를 수 있음을 보였다. Ma 등 [19]은 235B 교사 모델의 사고 연쇄를 2B 학생 모델로 LoRA 증류하여 PMC-VQA에서 64.9%를 달성했다. Rezaei 등 [20]은 7B~11B 모델군에 LoRA와 AdaLoRA를 비교 적용해 VQA-RAD·SLAKE에서 90~93%에 도달했고 15GB VRAM 환경에서의 배치 가능성을 함께 보고했다. 이들은 본 연구와 자원 제약이라는 문제의식을 공유한다. 다만 **하이퍼파라미터는 대체로 수작업이나 소규모 비교로 정해지며, 탐색 전략 자체를 연구 대상으로 삼지는 않는다.**

**LLM 에이전트 기반 HPO 연구(2.4.2)는 빠르게 늘고 있으나 결론이 엇갈린다.** AgentHPO [21]는 12개 대표 기계학습 과제에서 LLM 에이전트가 무작위 탐색을 상회하고 상당수 과제에서 사람의 최고 시도까지 넘어섰다고 보고했다. LLAMBO [22]는 베이지안 최적화의 구성 요소(warm start·대리 모델·후보 표집)를 LLM으로 대체하여, 관측이 희박한 탐색 초기에 이점이 있음을 보였다. 반면 Ferreira 등 [23]은 고정 연산 예산 아래 소형 언어모델을 튜닝하는 조건에서 CMA-ES·TPE 같은 고전 최적화가 순수 LLM 방식을 앞섰고 고전 최적화에 LLM 안내를 결합한 하이브리드가 가장 좋았다고 보고하면서, LLM은 고전 최적화의 대체재가 아니라 보완재로 가장 효과적이라고 결론지었다. 즉 **이 흐름은 아직 합의에 이르지 못했으며, 특히 의료 도메인 VLM 튜닝이라는 구체적 과제에서 동일 조건으로 비교한 사례는 드물다.**

**Table 2.1. 선행 연구와 본 연구의 위치 비교**

| 연구 | 모델 규모 | 도메인 적응 방식 | 평가 데이터셋 | 하이퍼파라미터 결정 |
|---|---|---|---|---|
| LLaVA-Med [9] | 7B | 의생명 코퍼스 기반 instruction 튜닝 | PathVQA·SLAKE·VQA-RAD | 수동 |
| Med-Flamingo [10] | OpenFlamingo-9B 기반 | 계속 사전학습 + few-shot | 의료 VQA(few-shot 설정) | 수동 |
| CheXagent [11] | foundation model | 흉부 X-ray 특화 사전학습 | 흉부 X-ray | 수동 |
| Shourya 등 [18] | 3B | LoRA 2단계 파인튜닝 | SLAKE·PMC-VQA·ROCO v2.0·MedPix 2.0 | 수동 |
| Ma 등 [19] | 2B (교사 235B) | LoRA 사고 연쇄 증류 | PMC-VQA | 수동 |
| Rezaei 등 [20] | 7B~11B | LoRA / AdaLoRA | VQA-RAD·SLAKE | rank 소규모 비교 |
| AgentHPO [21] | 해당 없음 (HPO 연구) | 해당 없음 | 일반 기계학습 12과제 | LLM 에이전트 vs 무작위·사람 |
| LLAMBO [22] | 해당 없음 (HPO 연구) | 해당 없음 | 일반 도메인 HPO | LLM 강화 베이지안 최적화 |
| Ferreira 등 [23] | 소형 언어모델 | 해당 없음 | 소형 LM 튜닝 | 고전(CMA-ES·TPE) vs LLM vs 하이브리드 |
| **본 연구** | **2~3B급 4종** | **QLoRA** | **PathVQA·SLAKE·VQA-RAD** | **LLM 에이전트 vs Optuna·무작위·수동 (동일 예산 직접 비교)** |

Table 2.1이 드러내듯 선행 연구는 **모델 적응**과 **탐색 전략** 중 한쪽에 집중되어 있다. 경량 VLM 계열은 적응 방법을 다루되 하이퍼파라미터를 수동으로 고정하고, HPO 계열은 탐색 전략을 다루되 일반 도메인 과제를 대상으로 한다. 본 연구는 이 두 축이 만나는 지점 — **소비자 GPU 환경 · 경량 VLM · QLoRA 도메인 적응 · LLM 기반 자율 HPO** — 에 위치하며, 차별점은 세 가지다.

첫째, 경량 VLM 계열 선행 연구가 대체로 단일 모델을 다루는 것과 달리 **2~3B급 공개 모델 4종을 3개 데이터셋에 대해 동일 프로토콜로 비교**하고, 파인튜닝 효과를 혼합효과모형과 부트스트랩으로 검정한다(RQ1, RQ2). 둘째, **하이퍼파라미터 탐색 전략 자체를 실험 변수로 삼아** autoresearch 스타일 자율 탐색을 Optuna(TPE)·무작위 탐색·수동 설정과 동일 예산 아래 직접 비교한다(RQ3). 셋째, Ferreira 등 [23]이 일반 도메인 소형 언어모델에서 관측한 고전 최적화의 우위가 **의료 VLM의 QLoRA 튜닝에서도 재현되는지**를 독립적으로 검증한다. 제4장 4.3의 결과는 이 물음에 대한 본 연구의 답이며, 제5장 5.2에서 [23]의 결론과 대조한다.
...
측정 지표는 Closed-ended accuracy(선택형), Open-ended accuracy(정규화 후 완전 일치 또는 정답 문자열 포함) 및 병기 지표인 BERTScore F1(roberta-large), 각 정확도의 부트스트랩 95% CI, 응답 시간(ms/문항), Peak VRAM(MB)이다.
...
#### 3.8.1 주관식 채점 기준과 BERTScore 이중 보고

Open-ended 응답의 정답 판정은 **정규화 후 완전 일치 또는 정답 문자열 포함**을 기준으로 한다(`src/evaluate/metrics.py`의 `compute_open_accuracy`). 제4장이 보고하는 주관식 정확도와 이를 포함한 전체 정확도, 그리고 그 위에서 수행한 모든 통계 검정은 이 기준을 따른다.

BERTScore F1은 정답 판정에 관여하지 않는 **병기 지표**다. 범용 기준(roberta-large, threshold ≥ 0.7)을 primary로, 의료 특화 기준(BioBERT, dmis-lab/biobert-v1.1)을 secondary로 함께 보고하되 이중 게이팅(두 지표 모두 통과해야 정답 처리)은 하지 않는다. 다만 roberta-large는 짧은 의료 답변에 관대해 임계값 통과율이 Phase 2 main 36조건 전부에서 99.91% 이상(34조건은 100%)으로 포화되므로, 이 지표는 모델·조건 간 변별력을 갖지 못한다. 정답 판정에 BERTScore 통과 여부가 아니라 위의 일치·포함 기준을 쓰는 이유가 여기에 있다.
...
가중치는 임상 중요도에 따라 diagnosis=1.0(진단 오류는 치료 방향에 직접 영향) > location=0.8 > measurement=0.7 > description=0.6 > temporal=yes_no=0.5(정보량이 제한적인 이진 판단) 순으로 부여했다.

**가중의 원리는 임상 품질관리의 확립된 관행에 근거한다.** 영상의학 판독의 표준 동료평가 체계인 ACR RADPEER는 판독 불일치를 단순 계수하지 않고 **임상적 유의성(clinical significance)에 따라 등급을 나눈다** — 임상적으로 유의한 불일치와 유의하지 않은 불일치를 구분하고, 사례의 난이도까지 반영해 분류한다 [17]. 즉 "모든 오류가 같은 무게를 갖지 않으며 환자의 치료 경로에 미치는 영향에 따라 차등해야 한다"는 전제는 본 연구가 새로 도입한 것이 아니라 임상 현장의 품질 평가에서 이미 통용되는 원칙이다. 진단명 오류를 위치·크기 서술 오류나 이진 판단 오류보다 무겁게 두는 본 연구의 순서는 이 원칙을 질문 유형 축으로 옮긴 것이다.

**다만 개별 가중치의 수치까지 문헌이 뒷받침하는 것은 아니다.** RADPEER가 제공하는 것은 등급화의 원리와 범주이지 유형별 수치가 아니며, 본 연구는 의료 전문가 패널이나 Delphi 합의를 거치지 않았다. 따라서 이 값들은 순서 관계를 표현하는 서수적 척도로 보아야 하며 절대적 임상 중요도의 측정값으로 해석할 수 없다. 이 한계가 결론에 영향을 주는지는 가중치를 바꿔가며 별도로 검증했고(4.4.4 Table 4.4a), 그 결과 WCA에 근거한 결론은 특정 가중치 조합에 의존하지 않음을 확인했다. WCA는 정확도를 대체하는 지표가 아니라 오류 패턴을 읽기 위한 보조 지표로만 사용한다.
...
> Overall Acc = Closed(객관식형)과 Open(주관식형) 문항을 합산한 정확도. Open 문항은 정규화 후 완전 일치 또는 정답 문자열 포함 기준으로, Closed 문항은 정답 문자열 일치 기준으로 채점했다(3.8.1). BERTScore F1 열은 정답 판정에 관여하지 않는 병기 지표이며, roberta-large 기준 문항별 F1의 평균이다.
...
학습 데이터 비율을 달리했을 때의 성능과 학습 시간은 Table 4.2a와 같다.

**Table 4.2a. 학습 데이터 비율별 성능 및 학습 시간 (Qwen3-VL-2B, PathVQA, 3시드 평균)**

| subset_ratio | 학습 샘플 수 | Overall Acc (평균) | 학습 시간(분, 평균) | 시간 SD |
|:---:|:---:|:---:|:---:|:---:|
...
반면 학습 시간은 데이터를 20배(982→19,654건) 늘려도 27.6~28.5분으로 사실상 일정하다. 본 연구가 `max_steps=500` 상한을 적용해 조건당 samples_seen을 4,000으로 고정했기 때문이며(3.6), 데이터 비율이 커질수록 늘어나는 것은 학습량이 아니라 **모델이 접하는 데이터의 다양성**이다. 즉 Table 4.2a의 정확도 향상(0.4150→0.5019, +8.7%p)은 추가 연산 없이 다양성만으로 얻어진 것으로, 고정 예산 환경에서는 학습 데이터 확보가 연산 확보보다 비용 대비 효과가 크다는 점을 시사한다. 다만 이 해석은 고정 스텝 예산이라는 조건에 한정되며, 상한을 해제해도 같은 관계가 유지되는지는 검증하지 않았다(5.3 (12)).

> **학습 시간 측정 기준**: 위 표의 학습 시간은 Trainer가 내부적으로 기록하는 `train_runtime_sec`를 분 단위로 환산한 값이다. `phase2_summary.csv`의 `train_time_min` 컬럼은 학습을 감싸는 스크립트가 wall-clock으로 재는 값이라 (모델, 데이터셋) 조합의 전처리 캐시를 **처음 생성하는 조건**에서 1회성 캐시 생성 비용이 합산돼 과대 측정된다. Phase 2 전체 75개 조건을 대조한 결과 이에 해당하는 조건은 9개였고 **전부 seed=42**였다(최대 편차 `qwen25-vl-3b/pathvqa/seed42`: 기록값 395.4분 vs 실측 44.9분). 정확도·loss 등 학습 결과 지표는 시간 측정 방식과 무관하므로 본 절의 정확도 기반 결론에는 영향이 없다. Phase 3은 애초에 `train_runtime_sec` 기준으로 시간을 기록하므로(`src/autoresearch/loop.py`) 4.3의 시간 수치는 이 문제의 영향을 받지 않는다. 재집계 스크립트는 `scripts/recompute_phase2_train_time.py`, 75조건 전수 대조표는 `results/phase2_finetune/train_time_corrected.csv`에 있다.
...
이 셋째 판단은 WCA 가중치를 어떻게 잡느냐에 따라 달라질 수 있으므로, 가중 체계를 바꿔가며 별도로 확인했다. 3.8.3에서 밝힌 대로 개별 가중치의 수치는 문헌으로 뒷받침되지 않는 서수적 척도이기 때문이다.

**Table 4.4a. WCA 가중치 민감도 (Qwen3-VL-2B, PathVQA 평가 500건)**

| 가중 체계 | 가중치 구성 | 제로샷 WCA | 최고 설정 WCA | 증가폭 | 전체 증가폭 대비 |
|---|---|:---:|:---:|:---:|:---:|
| 현행 (3.8.3) | 1.0 / 0.8 / 0.7 / 0.6 / 0.5 | 0.1527 | 0.2627 | +0.1099 | 0.87배 |
| 균등 | 전 유형 1.0 | 0.1843 | 0.2837 | +0.0995 | 0.79배 |
| 강한 대비 | 1.0 / 0.9 / 0.8 / 0.4 / 0.2 | 0.1119 | 0.2466 | +0.1348 | 1.07배 |
| 약한 대비 | 1.0 / 0.95 / 0.9 / 0.85 / 0.8 | 0.1738 | 0.2783 | +0.1045 | 0.83배 |
| 역순 | 0.5 / 0.5 / 0.5 / 0.6 / 1.0 | 0.2166 | 0.2915 | +0.0749 | 0.59배 |

> 가중치 구성은 diagnosis / location / measurement / description / yes_no 순이다. 역순 체계는 현실적 가정이 아니라 결론이 가중치 순서에 의존하는지를 확인하기 위한 반대 방향 점검이다. 전체 증가폭 대비는 WCA 증가폭을 전체 정확도 증가폭(+0.1260)으로 나눈 값이다. 재현 명령은 `scripts/wca_weight_sensitivity.py`, 산출물은 `results/phase3_autoresearch/wca_weight_sensitivity.json`이다.

**다섯 체계 전부에서 WCA는 증가했고(+0.0749 ~ +0.1348), 증가폭은 전체 정확도 증가폭의 0.59~1.07배 범위에 있다.** 임상 중요도 대비를 현행보다 키우면 오히려 증가폭이 커지고(1.07배), 가중치 순서를 완전히 뒤집어도 증가는 유지된다(0.59배). 즉 "임상 중요도로 가중해도 향상이 상쇄되지 않는다"는 판단은 특정 가중치 조합의 산물이 아니다. 다만 이 검증이 보장하는 것은 **결론의 가중치 비의존성**이지 가중치 자체의 임상적 타당성이 아니며, 앞서 지적한 표본 부족(diagnosis 3건, measurement 4건, temporal 0건)은 어떤 가중 체계로도 해소되지 않는다.
...
- **Closed-ended는 그대로 비교 가능하다.** 양측 모두 정답 문자열 일치 기반의 단순 정확도를 보고하며, 평가 표본도 SLAKE 1,061문항·VQA-RAD 451문항으로 동일하다(PathVQA만 6,761 vs 6,719로 42문항 차이).
- **Open-ended는 재채점을 거쳐 비교 가능하게 만들었다.** LLaVA-Med는 *생성 응답에 포함된 정답 토큰의 비율(recall)* 을 open 지표로 쓰는 반면, 본 연구가 4장 전반에서 보고하는 주관식 정확도는 *정규화 후 완전 일치 또는 정답 문자열 포함* 기준이다(3.8.1). 후자는 정답 전체가 그대로 담겨야 정답으로 인정하는 이분 판정이라 부분 일치를 인정하는 recall보다 엄격하므로, 두 수치를 그대로 나란히 두면 본 연구가 부당하게 불리해진다. 이에 **LLaVA-Med v1.0.0의 채점 함수를 그대로 재현해 본 연구의 주관식 응답을 동일 기준으로 다시 채점했다**(부록 C). 평가 자체를 다시 수행한 것이 아니라 저장해 둔 생성 응답에 채점 기준만 바꿔 적용한 것이므로 모델·표본·시드는 4장의 다른 수치와 동일하다.

**Table 4.4b. 선행 연구 보고 수치와의 간접 비교**
...
| *Open-ended (recall, 동일 기준)* | | | | | |
| LLaVA-Med | 7B | 의생명 코퍼스 사전학습 + 데이터셋별 파인튜닝 | **37.95** | **83.08** | **61.52** |
| **본 연구 (Qwen3-VL-2B)** | **2B** | **QLoRA 파인튜닝만** | 19.97 | 72.87 | 34.32 |

> LLaVA·LLaVA-Med 수치는 원논문 Table 4(a)에서 인용했다. 본 연구 closed 수치는 Phase 2 main 조건(3시드 평균)의 `closed_acc`이며, open 수치는 동일 조건의 생성 응답을 LLaVA-Med 기준으로 재채점한 3시드 평균이다(시드별 표준편차 0.11~0.34, 부록 C). 참고로 본 연구가 4장 다른 절에서 보고하는 일치·포함 기준 주관식 정확도는 PathVQA 17.15 / SLAKE 66.95 / VQA-RAD 26.00으로, 기준을 바꿔도 순위와 격차의 방향은 달라지지 않는다.
...
**그러나 동일 기준으로 맞춘 open-ended에서는 이 대등함이 성립하지 않는다.** 세 데이터셋 모두 LLaVA-Med가 앞서며, 격차는 PathVQA 17.98%p(19.97 vs 37.95), SLAKE 10.21%p(72.87 vs 83.08), VQA-RAD 27.20%p(34.32 vs 61.52)다. closed에서 사실상 동률이던 SLAKE조차 open에서는 10%p 뒤진다. 두 과제 유형이 요구하는 능력이 다르기 때문으로 보인다 — closed는 제한된 선택지 안에서의 판별이므로 QLoRA 적응만으로도 데이터셋의 답변 분포를 상당 부분 학습할 수 있으나, open은 정답 표현을 스스로 생성해야 하므로 의생명 코퍼스 사전학습에서 얻는 도메인 어휘와 표현 관습이 직접적으로 작용한다. 따라서 **"2B QLoRA 모델이 7B 의료 특화 모델과 대등하다"는 해석은 closed-ended에 한정해야 하며, 서술형 응답 품질까지 확대할 수 없다.** 이는 소비자급 환경에서의 경량 VLM 적응이 갖는 실질적 한계 지점을 가리킨다.

이 비교에는 다음 제약이 있으므로 결론을 확대 해석해서는 안 된다. (1) 본 연구 수치는 3시드 평균이나 LLaVA-Med는 단일 실행 보고값이다. (2) 학습 예산이 크게 다르다(본 연구는 `max_steps=500` 상한). (3) 동일 코드·동일 환경에서 재현한 것이 아니라 논문 보고값을 인용한 것이므로, 전처리·프롬프트 등 보고되지 않은 차이가 존재할 수 있다. (4) open-ended 재채점은 **채점 함수만** 동일하게 맞춘 것이다. LLaVA-Med의 보고값은 자체 평가 파이프라인에서 산출된 것이므로 답변 생성 조건과 평가 표본 구성(PathVQA 6,761 vs 6,719)의 차이는 남아 있다. 동일 프로토콜 하의 직접 재현 비교는 향후 과제로 남는다(5.3(2)).
...
이 결과는 **도메인이 다른 독립 연구와 같은 방향을 가리킨다.** Ferreira 등 [23]은 고정 연산 예산 아래 소형 언어모델을 튜닝하는 조건에서 CMA-ES·TPE 같은 고전 최적화가 순수 LLM 방식을 앞섰고, 고전 최적화에 LLM 안내를 결합한 하이브리드가 가장 좋았다고 보고했다. 본 연구가 의료 VLM의 QLoRA 튜닝이라는 다른 과제에서 얻은 결론 — 자율 탐색이 Optuna를 넘어서지 못했고, 설정 불일치를 교정한 뒤에도 우위는 확인되지 않았다 — 은 이와 일치한다. 서로 다른 도메인·모델·탐색 공간에서 같은 방향의 결과가 나왔다는 점은 **LLM 에이전트를 기존 최적화의 대체재로 보는 관점에 대한 반증이 특정 과제의 우연이 아닐 가능성**을 높인다. 다만 두 연구 모두 예산이 제한된 조건에서 수행되었으므로, 예산을 크게 늘렸을 때의 관계는 여전히 열려 있다(5.3(13)).
...
**(2) 의료 특화 VLM과의 직접 비교 부재.** LLaVA-Med, Med-Flamingo, CheXagent 등 의료 특화 VLM을 본 연구의 환경에서 직접 재현해 비교하지는 않았다(2.5). 4.4.6의 Table 4.4b는 LLaVA-Med가 **동일한 세 데이터셋의 표준 test split**에 대해 보고한 수치를 인용한 간접 비교이며 다음 세 가지 제약을 안는다. 첫째, **채점 기준을 맞춘 범위에서만 비교가 성립**한다 — closed-ended는 양측이 같은 정확도 기준을 쓰므로 그대로 비교되고, open-ended는 LLaVA-Med v1.0.0의 채점 함수를 재현해 본 연구 응답을 동일 기준(recall)으로 다시 채점한 값을 쓴다(부록 C). 다만 맞춘 것은 채점 함수뿐이며 답변 생성 조건과 평가 표본 구성의 차이는 남는다. 둘째, 본 연구 수치는 3시드 평균이나 인용한 수치는 단일 실행 보고값이며 학습 예산도 크게 다르다. 셋째, 동일 코드·환경에서 재현한 것이 아니므로 전처리·프롬프트 등 보고되지 않은 차이가 남아 있다. Med-Flamingo·CheXagent는 이 세 데이터셋에 대해 동일 프로토콜의 수치를 제공하지 않아 표에서 제외했다. 동일 프로토콜 하의 직접 재현 비교는 향후 과제로 남는다.
...
**(7) 임상적 의미 평가의 간접성.** Weighted Clinical Accuracy(WCA)에서 오류를 임상적 유의성에 따라 차등한다는 **원리**는 영상의학 동료평가 체계인 ACR RADPEER [17]가 채택한 확립된 관행에 근거하나, **개별 유형에 부여한 수치**는 의료 전문가 합의를 거치지 않은 서수적 척도이므로(3.8.3) 절대적 임상 중요도의 측정값으로 해석할 수 없다. 이 한계가 결론을 좌우하는지는 가중 체계 다섯 가지로 검증했고, WCA 증가는 역순 가중을 포함한 전 체계에서 유지되어 결론이 특정 가중치에 의존하지 않음을 확인했다(Table 4.4a). 다만 이는 결론의 가중치 비의존성을 보인 것이지 가중치의 임상적 타당성을 입증한 것은 아니다. Expected Calibration Error(ECE)는 현재 평가 파이프라인이 per-sample confidence를 저장하지 않아 산출하지 못했다. 4.4.4에서 제기한 "정확도 향상이 임상적 가치가 낮은 유형에 편중되었을 가능성"은 810 trial 전수의 유형별 분해로 검증했고(Table 4.4), 편중 자체는 확인되었다. 그러나 검증의 결정적 제약은 평가 표본의 유형 구성에 있다 — 평가 500건에서 diagnosis는 3건, measurement는 4건, temporal은 0건이므로 **최상위 가중치 유형의 성능은 사실상 판정되지 않았다.** 이 제약은 본 연구의 평가 설계가 아니라 PathVQA의 유형 분포 자체에서 비롯되며(전체 6,719건 중 diagnosis 23건, 0.34%), 해소하려면 임상 중요 유형을 과표집한 별도 평가셋이 필요하다(5.4).
...
> **표기 주석**: 저자가 6인을 넘는 [2]·[3]·[4]·[9]·[10]·[11]·[15]·[17]은 IEEE 규정에 따라 제1저자만 적고 "et al."로 줄였다(전체 공저자 목록은 각 문헌 원문에서 확인할 수 있다). [2]·[3]·[6]·[11]·[18]·[19]·[21]·[23]은 학술대회 게재본이 없는 기술 보고서 또는 프리프린트다. [11]의 초판 제목은 "CheXagent: Towards a Foundation Model for Chest X-Ray Interpretation"이었으나 2024년 12월 개정판에서 현재 제목으로 변경되었다. 본문 2.5의 CheXagent가 이 문헌이다. [17]-[23]은 2026년 8월 원문 또는 서지 레코드(JACR 공식 페이지, arXiv 초록 페이지, Crossref)를 직접 대조해 확인했다.
...
### 부록 C. LLaVA-Med 기준 주관식 재채점

4.4.6의 open-ended 비교를 위해 본 연구의 주관식 응답을 LLaVA-Med와 동일한 기준으로 다시 채점한 절차와 결과를 정리한다.

**(1) 재채점이 가능한 이유**

본 연구의 평가 파이프라인은 조건별로 문항 단위 기록(`per_sample`)을 남긴다. 각 기록은 질문, 정답, **모델이 생성한 응답 원문**, 문항 유형을 포함하므로, 채점 기준만 바꿔 다시 적용하면 모델을 재실행하지 않고도 다른 지표를 산출할 수 있다. GPU 재실행은 필요하지 않았다.

**(2) 채점 기준**

LLaVA-Med v1.0.0의 `llava/eval/eval_metrics/evaluate_metrics.py`에 정의된 `calculate_f1score`의 recall 성분을 사용한다. 정답과 생성 응답을 VQA 표준 절차로 정규화(소문자화, 구두점·관사 제거, 축약형·수사 표준화)한 뒤 공백 단위로 토큰화하고, 정답 토큰 중 생성 응답에 나타난 비율을 구한다. 구현상의 비대칭(공유 토큰은 응답 쪽 빈도로, 누락 토큰은 정답 쪽 빈도로 계산)까지 원본을 그대로 따랐다 — 비교 가능성이 목적이므로 더 타당해 보이는 변형을 도입하면 안 되기 때문이다.

재현 정확성은 원본 함수를 참조 구현으로 옮겨 두고 차분 대조하여 확인했다. Phase 2 main 조건의 주관식 응답 12,797건 전수와 경계 사례 8건에서 두 구현의 값이 완전히 일치했다(불일치 0건).

**(3) 결과**

**Table C.1. LLaVA-Med 기준 주관식 재채점 (Qwen3-VL-2B, Phase 2 main, 3시드)**

| 데이터셋 | 주관식 문항 수 | 본 연구 recall (평균) | SD | LLaVA-Med 보고값 | 격차 |
|---|:---:|:---:|:---:|:---:|:---:|
| PathVQA | 3,357 | 19.97 | 0.11 | 37.95 | −17.98 |
| SLAKE | 706 | 72.87 | 0.31 | 83.08 | −10.21 |
| VQA-RAD | 200 | 34.32 | 0.34 | 61.52 | −27.20 |

시드 간 표준편차가 0.11~0.34로 작아, 관측된 격차(10~27%p)가 시드 변동으로 설명되지 않음을 확인할 수 있다.

**(4) 참고: 채점 기준별 주관식 수치 비교**

| 데이터셋 | 일치·포함 기준 (본문 지표) | LLaVA-Med recall 기준 | BERTScore F1 ≥ 0.7 통과율 |
|---|:---:|:---:|:---:|
| PathVQA | 17.15 | 19.97 | 100.00 |
| SLAKE | 66.95 | 72.87 | 100.00 |
| VQA-RAD | 26.00 | 34.32 | 100.00 |

세 기준의 관대함은 BERTScore 임계값 > recall > 일치·포함 순이다. recall은 부분 일치를 인정하므로 정답 전체가 담겨야 인정하는 일치·포함 기준보다 2.8~8.3%p 높게 나오며, 데이터셋 간 상대적 순위(SLAKE ≫ VQA-RAD > PathVQA)는 두 기준에서 동일하다. 반면 BERTScore 통과율은 Phase 2 main 36조건 전부에서 99.91% 이상으로 포화되어 세 데이터셋이 모두 100.00으로 붙으므로 순위 자체가 형성되지 않는다. 이 지표를 정답 판정 기준으로 쓸 수 없는 이유가 여기에 있다(3.8.1).
...
채점 함수는 `src/evaluate/llavamed_recall.py`, 정규화 테이블은 출처를 밝혀 `src/evaluate/vendor/llavamed_glossary.py`에 인용해 두었다. 재현 검증 테스트는 `tests/test_llavamed_recall.py`, 시드별 원시 결과는 `results/phase2_finetune/llavamed_recall_rescore.json`에 있다.

---

*(참고문헌 23건은 2026년 8월 원문 대조 완료. 남은 작업: 인용 표기 형식을 학과 지정 양식으로 통일)*
