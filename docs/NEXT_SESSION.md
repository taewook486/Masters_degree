...
> 이 파일이 **유일한 인계 문서**다. 2026-09-03에 루트 `NEXT_SESSION.md`를 삭제하고
> 여기로 합쳤다. 그 파일이 담고 있던 8/24~8/30 시점의 중간판은 git 히스토리에만
> 남아 있다 — 필요하면 `git log -p -- NEXT_SESSION.md`로 볼 것.
...
## 🟢 2026-08-23 — 팟 Terminate 완료. 프로젝트 잔여 작업 없음

**논문은 (3)절 상태 그대로 완료고, 마지막 남아 있던 팟 처리를 끝냈다. 이제 할 일이 없다.**

### 바로 지웠으면 조용히 잃을 뻔했다

지우기 전에 팟을 켜서 `/workspace` **91,073개 파일을 전수 열거**해 로컬과 대조했다. 결과는 "차이 0"이 아니었다 — 로컬에 없는 게 **67건** 나왔다.

| 누락 | 규모 |
|---|---:|
| Phase 3 v1 `run_*.log` | 59건 / 19.7MB |
| v2 `results_failed_20260816.tsv` (실패 trial 기록) | 201KB |
| v2 `results.tsv.counter` | 4B |
| wandb 디버그 로그 | 5건 |
| 데이터셋 다운로드 로그 | 1건 |

전부 받아서 3단계 검증(개수 → 매니페스트 → 바이트 합계)을 통과시켰다. 67/67 파일, **20,871,522 B 양쪽 동일**.

> 교훈: **매니페스트 일치는 "받기로 정한 건 다 받았다"는 증명이지 "목록 자체가 완전했다"는 증명이 아니다.** 8/21에도 평가 json을 하마터면 목록에서 빠뜨릴 뻔했었다. 되돌릴 수 없는 삭제 전에는 원본을 전수 열거할 것. 이번 대조에 든 GPU 비용은 $0.3 정도다.

### v1 어댑터는 이미 사라져 있었다 (이번 삭제와 무관)

610 trial 중 어댑터 디렉터리는 611개 남아 있었지만 **실제 가중치가 있는 건 manual 11개뿐**이었다. optuna 최고 trial_0304(0.4700)조차 config·tokenizer 껍데기만 남고 `adapter_model.safetensors`가 없었다. 8/11 디스크 위기 때 지운 것이다.

즉 **조건 간 가중치 비교는 이 시점에 이미 불가능**했고, 이번 Terminate가 없앤 게 아니다.

### v2 200개 전량 수집은 하지 않았다

처음엔 "$1.4면 싸니까 전부 받자"고 판단했는데 **싸다는 건 값어치의 근거가 아니었다.** 실제로 따져보니:

- v2 200개는 **전부 `autoresearch_v2` 단일 전략**이다. 하이퍼파라미터만 다른 표본이고 그 차이는 `results.tsv`에 수치로 다 있다
- 비교 대상인 v1 가중치가 없으니 200개를 받아도 조건 간 분석이 안 된다
- 논문 수치는 전부 평가 json `per_sample`에서 나오고, 그건 **810 trial 전부 로컬에 있다**

어댑터가 있어야만 되는 건 "새 입력에 대한 추론" 하나뿐이고, 그건 최고 모델 하나면 된다.

### 최종 보유 자산 (`results_pod_backup/`, 전부 .gitignore 대상)

| 자산 | 내용 |
|---|---|
| v2 상위 3개 어댑터 | trial_1030(0.4780) / 0880(0.4740) / 1027(0.4740), 각 379,669,720 B |
| v1 manual 11개 | 각 12,861,664 B — **현존하는 유일한 v1 가중치** |
| 평가 json `per_sample` 500건 | **810/810 trial 전부** (v1 610 + v2 200) |
| 신규 디렉터리 2개 | `pod_final_sweep_20260823/`(67건), `adapters_extra_20260823/`(89건) — 각각 매니페스트 2종 동봉 |

팟은 `list-pods` 빈 배열로 소멸 확인. **이후 과금 $0.**

### RunPod SSH 접속 함정 (다음에 팟 쓰면 바로 적용)

1. **프록시 `ssh.runpod.io`는 PTY를 요구**해서 배치 모드로 명령을 못 보낸다(`Error: Your SSH client doesn't support PTY`). `-tt`를 줘도 세션이 안 닫히고 멈춘다. → 팟이 RUNNING이 되면 `get-pod` 응답의 `ssh.direct`에 직접 접속 정보가 뜨므로 **그쪽(`root@<ip> -p <port>`)을 쓸 것.** 포트는 재시작마다 바뀐다(이번엔 40020).
2. 키는 `runpod_openssh.pem`(저장소 루트, `.gitignore:131` `*.pem`으로 차단됨). DrvFs라 권한이 777이어서 ssh가 거부한다 → **리눅스 파일시스템에 복사 후 `chmod 600`.** 실효 속도 4.5MB/s.
3. `pkill -f "<패턴>"`을 Bash 한 줄에 쓰면 **그 명령줄 자체가 패턴에 매치돼 자기 자신을 죽인다**(exit 144). `[p]attern` 형태로 쓸 것.

### 환경 정리

`moai update`가 되돌려 놓은 `language.yaml`(ko → en)을 또 복구했다. `git index.lock` 잔재(0바이트, 10분 경과, 쓰기 프로세스 없음)도 확인 후 삭제했다.

---

## 🟢 2026-08-22 (4) — 세션 정리 (논문 작업 없음)

**논문 상태는 아래 (3)절 그대로다. 이 절은 환경 정리 기록이다.**

### MoAI 툴킷 업데이트가 한국어 설정을 되돌려놨다

오늘 **17:58에 `moai update`가 돌면서 492개 파일을 덮어썼고**, 그 과정에서 사용자 설정 두 개가 템플릿 기본값으로 초기화됐다.

| 파일 | 되돌아간 값 | 복구값 |
|---|---|---|
| `.moai/config/sections/language.yaml` | `conversation_language: en` | **`ko` / `Korean`** |
| `.moai/config/sections/user.yaml` | `name: ""` | **`taewook`** |

`git checkout --`로 복구했다. **다음에 `moai update`를 돌리면 또 덮인다** — 업데이트 직후 이 두 파일의 diff를 반드시 확인할 것.

### git index.lock 잔재

`git checkout`이 `index.lock` 때문에 실패했다. 확인해 보니 **14:13:51 — 그날 마지막 커밋 직후에 남은 0바이트 잔재**였고(6시간 반 전), 그 시점에 돌던 git 프로세스는 statusline의 읽기 전용(`diff`·`status`)뿐이었다. 삭제하고 진행했다.

> 판단 근거: 락 파일 mtime과 마지막 커밋 시각이 11초 차이 → 쓰기 중이 아니라 잔재. `ps`로 쓰기 프로세스 부재를 확인한 뒤에만 지울 것.

### 툴킷 업데이트를 커밋했다

492개 변경(수정·삭제·신규)을 chore 커밋 하나로 기록했다. **논문 파일은 전혀 건드리지 않았다.** 문제가 생기면 이 커밋 하나만 revert하면 된다.

`.gitignore`에 두 줄을 추가했다.
- `~$*` — Word 임시 잠금 파일(문서를 열어두면 생긴다)
- `.moai/reports/session-*.md` — 세션별 일회성 리포트

### 종료 시점 주의

세션 종료 시각에 **Word가 국문 논문 docx를 열어둔 상태**였다(`~$학위논문_국문.docx` 잠금 파일 존재). 컴퓨터를 끄기 전 Word를 저장·종료해야 한다.

### 국문 표지 수정 — 이름이 다음 쪽으로 넘어가던 문제 (커밋 `c581058`)

사용자가 Word에서 표지 개행 하나를 지웠다(9 → 8). 그 전에는 **저자 이름이 표지를 넘어 다음 페이지로 밀려나 있었다.** 수정된 docx로 PDF도 재생성했다(84쪽).

본문은 무변경임을 확인했다 — 문단 431개 / 표 30개 / 69,835자가 해시 단위로 일치한다. docx가 203KB → 133KB로 줄어든 것은 Word 재저장이 python-docx 생성 시 쌓인 중복 마크업(`document.xml` 4.37MB → 924KB)을 정리한 결과이며 내용 손실이 아니다.

> `build_thesis_docx.py`는 표지 개행을 9개로 넣으므로 **재빌드하면 재발한다.** 스크립트 표지 생성부를 8개로 고치는 것이 근본 해결이다.

### 영문 표지도 해결했다 — 원인은 행 최소 높이였다

**해결값: 표지 셀 개행 2 + `r2` trHeight 5000 twips.** 표지 8행(r0~r7)이 전부 1쪽에 들어오고 속표지(r8)는 2쪽 맨 위에서 시작한다. 102쪽으로 쪽수 변화 없다. PDF도 재생성했다.

본문 무결성 확인: 문단 561개 전수 일치, 표 29개·셀 809개 중 **바뀐 셀은 표지 하나뿐**이고 그것도 개행 개수만 다르다(개행 제외 텍스트 동일). 본문 해시 일치.

`r2` 높이별 `r7` 시작 위치 실측(개행 2 기준):

| trHeight | r7 시작 vpos | 결과 |
|---:|---:|---|
| 4000 | 548.4 | 1쪽 |
| **5000** | **577.85** | **1쪽 (채택)** |
| 5200 | 587.7 | 1쪽 |
...
임계가 603 근처라 경계에 붙이지 않고 **5000으로 약 25pt 여유**를 뒀다. 제출본이 다른 PC에서 열릴 때 글꼴이 대체돼도 견디게 하려는 것이다.

> 아래는 원인을 찾기까지의 경위다. 같은 실수를 반복하지 않으려고 남긴다.

**빈 줄 삭제로는 못 고친다.**

국문을 고치면서 영문도 확인해 보니 **같은 종류의 결함이 남아 있다.** `Graduate School of Information & Communications` 줄이 2쪽으로 밀려 있다(커밋본 기준으로는 `February, 2027`·`Department`까지 세 줄이 2쪽이었다).

**국문에서 통한 방법(빈 줄 삭제)이 영문에는 안 통한다.** 실측했다.

| 표지 셀 개행 | 2쪽으로 밀린 줄 |
|---:|---|
| 10 (커밋본) | February / Department / Graduate School |
| 7 | Department / Graduate School |
| 5 | Graduate School |
| 4 | Graduate School |
| 2 | Graduate School |

**5개 아래로는 아무 변화가 없다.** 원인은 빈 줄이 아니라 **표 행의 고정 최소 높이**다. 표지는 `tables[0]` 하나에 표지(r0~r7)와 속표지(r8~r15)가 함께 들어 있고 모든 행에 `w:trHeight`가 박혀 있는데, 제목 행 `r2`의 바닥값이 6337 twips(4.40인치)라 셀 내용이 짧아져도 행이 그 아래로 줄지 않는다.

개행만 10 → 2로 줄이는 시도를 먼저 했는데 마지막 한 줄을 못 끌어올렸다. **처음엔 "1쪽이 꽉 찼다"고 추정했지만 계산해 보니 아니었다** — 표지부 고정 높이 합은 7.58인치로 가용 8.15인치에 들어간다. 실제로는 `r2`의 렌더링 높이가 최소값(4.40인치)을 넘어서고 있었고, 개행을 5개 아래로 줄이면 이번엔 그 최소값이 바닥이 되어 더는 안 줄어드는 구조였다. 그래서 **개행과 높이를 함께** 줄여야 했다.

`keepNext`·`cantSplit`·명시적 페이지 나누기는 전부 없었다(확인함). 셀은 모두 `vAlign=center`라 빈 줄을 줄여도 제목 블록이 행 안에서 가운데 정렬돼 균형이 유지된다.

측정법과 함정은 메모리 `thesis-cover-row-height-floor.md`에 적어 뒀다. 요점: Word COM은 이 환경에서 **후기 바인딩 필수**이고 `Range.Information`은 메서드가 아니라 **매개변수 있는 속성**이다. `Information(3)`은 범위의 **끝** 쪽을 주므로, 시작 vpos가 1쪽인데 결과가 2쪽이면 그 행이 경계에 걸쳐 있다는 뜻이다. `Row.Height`는 설정된 최소값일 뿐 실제 높이가 아니므로 **모델링하지 말고 쪽 배치를 직접 잴 것.** COM 인스턴스가 남아 파일을 잠그니 쓰기 전마다 `taskkill /F /IM WINWORD.EXE`.

### 빌드 스크립트도 고쳤다 — 재빌드해도 재발하지 않는다

표지 결함은 양식 템플릿의 기본 간격을 그대로 쓴 데서 왔으므로, 산출물만 고치면 **다음 재빌드에서 그대로 되살아난다.** `build_thesis_docx.py`에 보정을 넣었다.

- 상수 3개를 파일 상단에 두고 근거를 주석으로 남겼다 — `COVER_BLANKS_KO=8`, `COVER_BLANKS_EN=2`, `COVER_TITLE_ROW_HEIGHT_EN=5000`
- 헬퍼 둘: `_trim_cell_blank_lines`(빈 문단만 지우므로 제목·성명은 안 건드린다), `_set_row_min_height`
- 국문은 셀을 **채운 뒤에** 줄인다 — 먼저 줄이면 `paragraphs[9]` 인덱스가 사라진다
- 영문은 제목 셀을 문구로 찾는다. 양식이 바뀌어 못 찾으면 빌드는 계속하되 **경고를 출력**한다(조용히 넘어가면 표지가 갈라진 채 나간다)

검증: 두 언어를 임시 경로로 재빌드해 Word로 쪽 배치를 실측했다. 국문 표지 5행·영문 표지 8행 모두 1쪽, 속표지는 2쪽 맨 위 시작, 84쪽/102쪽으로 쪽수 동일. **영문은 vpos 값이 수동 수정한 제출본과 완전히 같다**(198.25 / 434.8 / 467.55 / 522.7 / 577.85). 신규 빌드와 제출본의 표지 개행·행 높이·문구가 전부 일치함도 대조로 확인했다. 제출본 docx는 건드리지 않았다.

### 남은 작업

1. **팟 처리** — `rtml9bukx69vgz` Stopped 유지 중, 월 약 $13.7. 논문 최종 확정 후 Terminate 판단.
2. **보관 요율 확정치**(선택) — 8/23 이후 `get-billing scope=pods bucketSize=day`로 8/22 전체 버킷 확인.

---

## 🟢 2026-08-22 (3) — 논문 최종 검토 + 지적 3건 반영 (커밋 `30d3057`)

**논문 작업은 이것으로 끝났다.** 국·영문 전문을 훑어 검토했고, 나온 지적을 전부 반영해 제출본을 다시 만들었다.

### 검토에서 나온 것 — 초록이 논문 기여를 스스로 깎고 있었다

무결성 검사(참고문헌·절 참조·표 번호·수치 대칭)는 전부 통과했고, **실질 지적은 전부 "본문에는 있는데 초록·기여 절에 없다"** 유형이었다.

**A. 국문판 영문초록이 구버전이었다 (사실 오류).** 8/21에 §4.3.5를 반영할 때 영문판 ABSTRACT만 고치고 **국문판 뒤에 붙는 영문초록을 빠뜨렸다.** 본문은 전부 810인데 이 초록만 `610 trials`로 남아 있었다. 나머지 3개 문단은 동일했으므로 한 문장만 교체했다.

> 교훈: **같은 초록이 두 파일에 각각 존재한다**(국문판에 국문초록+영문초록, 영문판에도 둘 다). 초록을 고칠 때는 항상 4개를 함께 봐야 한다. 짝 대조 스크립트를 돌리는 게 안전하다.

**B. 초록 4종 모두 v2 결과가 없었다.** 규모(810)만 갱신되고 결론 서술은 원본 기준이라 "에이전트 열세 + 중복 제안이 원인"에서 끝났다. 본문 §5.1은 그 중복이 설정 불일치의 산물이었고 제거하면 열세가 해소된다고 보고하므로, **초록만 읽은 심사자는 본문보다 불리하게 읽게 된다.** 고유 설정 18.90/20과 p = .1726을 넣되, run-level 미달(0.4292 대 0.4490)과 원본 대비 개선 미입증(p = .2387)을 함께 적어 부정적 결론은 유지했다.

**C. §5.2 셋째 기여가 "진단 제시"에 머물러 있었다.** 실제로는 별도 200 trial 재실험으로 그 진단을 검증했다. "가설을 세웠다"와 "가설을 검증했다"는 기여의 급이 다르므로 격상하고, 프롬프트–구현 정합성을 통제 변수로 다뤄야 한다는 시사점을 덧붙였다.

### 무결성 검사 결과 (전부 통과)

| 항목 | 결과 |
|---|---|
| 참고문헌 | 국·영문 각 16개, 결번 0 · 미인용 0 · 목록에 없는 인용 0, IEEE 첫 등장 순서 준수 |
| 핵심 수치 29종 | 국·영문 빈도 대칭 (`810` 1건 차이는 번역 표현 차이 — 국문 "810 trial(Phase 3)" vs 영문 "810-trial programme") |
| 절 상호참조 | 헤딩 63개, 없는 절 참조 0건 |
| 표 | 양쪽 23개, 캡션 중복 0 |
| §5.3 / §5.4 | 15개 / 6개 양쪽 대칭 |
| 목차 중복 | md의 `## 목차`는 docx에 미포함(빌드가 제외) |

### 표 명시 참조도 보완 완료 (커밋 `50ba431`)

미참조 상태였던 표 18개를 본문에서 호출하도록 고쳤다. 세 유형으로 나눠 처리했다.

- **기존 문장에 참조만 삽입 (3건)** — 4.1a는 "다음과 같다"를 "Table 4.1a와 같다"로, 4.3d·4.3e는 문장 끝에 참조를 덧붙였다
- **헤딩 직후라 도입 문장이 아예 없던 것 (10건)** — 3.1·3.2·3.3·3.4·4.2·4.2a·4.2b·4.2c·4.3a·4.3b. 절의 목적을 한 문장으로 밝히고 표를 호출하는 문장을 신설했다
- **문맥이 끊겨 신설이 필요한 것 (5건)** — 4.2d·4.2e·4.3c·4.3f·4.3g

**내용은 더하지 않았다** — 캡션이 이미 말하는 바를 본문에서 호출하는 연결 문장만 넣었다.

검증: 미참조 국·영문 각 18건 → **0건**(표 23개 전수), 조판 회귀 없음, 삽입 문맥 육안 확인.

### 최종 상태

**제출본: 국문 84쪽 / 영문 102쪽.** 표 분리 0건, 미참조 표 0건, 초록 새 페이지 시작 유지, 목차 쪽번호 정합 유지.

**논문 작업은 이것으로 완료다.** 남은 것은 팟 처리뿐이다.
