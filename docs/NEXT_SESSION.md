...
표 23개 중 **18개가 본문에서 "Table X"로 명시 참조되지 않는다**(캡션만 두고 앞 문장이 "다음과 같다" 식으로 도입). 양쪽 언어가 동일한 기존 스타일이고 학위논문에서 흔히 허용되지만, 학술 관행상 명시 참조가 원칙이다. **지도교수 선호를 확인해 판단할 것.**

### 최종 상태

**제출본: 국문 84쪽 / 영문 101쪽** (수정 후에도 쪽수 불변). 표 분리 0건, 초록 새 페이지 시작 유지, 목차 쪽번호 정합 유지.
