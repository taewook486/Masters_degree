...
**시도했다가 되돌렸다.** 개행을 10 → 2까지 줄여 봤으나 마지막 한 줄을 못 끌어올렸고, 문제를 해결하지 못한 채 표지 간격만 바뀐 상태였으므로 **영문 docx를 커밋본으로 원복했다**(셀·문단 전수 일치 확인). 영문 제출본은 현재 8/22 커밋본 그대로다.

다음에는 빈 줄이 아니라 **`r2`의 `trHeight`를 줄여야 한다.** 표지부(r0~r7) 고정 높이 합은 7.58인치로 가용 8.15인치에 들어가야 하는데도 밀리므로, 머리글·바닥글 점유나 `tblpPr` 위치 지정도 함께 볼 것.

측정에 쓴 도구와 함정은 메모리 `thesis-cover-row-height-floor.md`에 적어 뒀다. 요점만: Word COM은 이 환경에서 **후기 바인딩 필수**이고 `Range.Information(3)`은 메서드가 아니라 **매개변수 있는 속성**이다. COM 인스턴스가 남아 파일을 잠그므로 python-docx로 쓰기 전에 `taskkill /F /IM WINWORD.EXE`가 필요하다.

### 남은 작업

1. **영문 표지 결함** — 위 진단대로 `r2` 행 높이로 접근할 것. 국문은 해결됨.
