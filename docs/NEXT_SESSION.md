...
- **2026-08-13 — Phase3 결과 잔여분 백업 + 팟 유휴 과금 사고 + 토큰 유출 조치**
  ...
  - **[팁] 품질 게이트 우회**: 저장소에 기존 ruff 부채 265건이 쌓여 있어 무관한 커밋도 막힘. `--no-verify`는 **안 통함**(git 훅이 아니라 Claude Code 훅). 정식 우회는 `MOAI_SYNC_GATE_BLOCKING=0 git commit ...`.
...
**[2026-08-15 갱신] 표 서식 검토·PDF 재변환·인용 형식 통일은 전부 끝났다. 남은 건 아래 4개이고, 1번 외에는 전부 사용자 액션이다.**

1. **새 제출본 육안 확인 (권장)** — 참고문헌이 IEEE로 바뀌었으니 `석사학위논문_국문.pdf` 끝부분을 한 번 볼 것. 확인 항목: ① `[1]`~`[16]` 번호가 본문 인용과 맞는지 ② 두 줄 이상 항목이 **둘째 줄부터 들여쓰기** 되어 있는지 ③ 게재처명이 이탤릭으로 나오는지(별표가 보이면 실패). **PDF를 이미지로 렌더링한 육안 확인은 이번에도 못 했다**(`poppler-utils` 미설치) — XML 수준 검증만 했다.
