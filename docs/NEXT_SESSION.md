...
> ⚠️ `build_thesis_docx.py`로 재빌드하면 **국·영문 표지 결함이 둘 다 재발한다.** 스크립트 표지 생성부(국문 개행 8, 영문 개행 2 + r2 높이 5000)를 고치는 것이 근본 해결이다.

### 남은 작업

1. **빌드 스크립트 표지 생성부 수정**(권장) — 재빌드 시 국·영문 표지 결함 재발 방지.
2. **팟 처리** — `rtml9bukx69vgz` Stopped 유지 중, 월 약 $13.7. 논문 최종 확정 후 Terminate 판단.
3. **보관 요율 확정치**(선택) — 8/23 이후 `get-billing scope=pods bucketSize=day`로 8/22 전체 버킷 확인.
