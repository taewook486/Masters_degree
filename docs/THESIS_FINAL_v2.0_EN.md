...
|----------|:---:|:---:|:---:|:---:|:---:|
| PathVQA | 4,998 | 32,632 | English | Pathology | Open + Closed (7 types) |
| SLAKE | 642 | 7,033 | English | Radiology / CT | Open + Closed |
| VQA-RAD | 315 | 2,244 | English | Radiology | Open + Closed |

Each dataset was used with its official train/val/test split unchanged. The QA-pair counts are measured from the public releases actually downloaded for this study (the figures published in the original papers are given alongside them in Section 3.4); all reported accuracies are computed on the full test split of each dataset (PathVQA 6,719 / SLAKE 1,061 / VQA-RAD 451; 8,231 items in total). See Section 4.1.
...
**The indirect comparison with a prior medical-specialized model located the limit of domain adaptation, and that limit falls differently across task types.** In the indirect comparison with LLaVA-Med, a 7B medical-specialized model, QLoRA adaptation of a lightweight 2B model reached comparable closed-ended accuracy on SLAKE (85.26% vs. 85.34%), but trailed by 8-11%p on closed-ended PathVQA and VQA-RAD and by 10-27%p on open-ended questions under a matched token-recall criterion. Parity therefore holds only for closed-ended questions, and only on SLAKE; for open-ended questions, where the model must generate the answer expression itself, the advantage conferred by large-scale biomedical pretraining is not offset by QLoRA adaptation alone (Section 4.4.6).
...
**(16) Restricted language scope.** SLAKE is a bilingual English-Chinese dataset, but this study used the English-only public release (`mdwiratathya/SLAKE-vqa-english`; Section 3.4). The conclusions therefore hold for English medical VQA, and generalization to multilingual settings including Chinese lies outside the scope of this verification. Because the extent of multilingual pretraining differs across models, whether the cross-model ranking observed in Section 4.1 is preserved in other languages requires separate verification.
